...
export async function generateReport(toothData, plaqueStats) {
  ...
  try {
    ...
    if (plaqueStats) {
      ...
      const ratio = plaqueStats.plaque_ratio != null ? `${(plaqueStats.plaque_ratio * 100).toFixed(1)}%` : '-';
      y = infoTable(doc, [
      ...
      for (const [fdi, info] of sorted) {
        ...
        doc.setFillColor(...BG); doc.roundedRect(M + LW, y, BW, BAR_H, 1, 1, 'F');
        if (pct > 0) { doc.setFillColor(...RED); doc.roundedRect(M + LW, y, BW * pct, BAR_H, 1, 1, 'F'); }
    ...
    for (const mode of modes) {
      try { b64s.push(await snap3D(mode)); } catch(e) { b64s.push(null); }
    }
    ...
    b64s.forEach((b64, i) => {
      ...
      if (b64) {
        ...
        doc.setFillColor(...BG); doc.rect(x, y, snapW, snapImgH, 'F');
        doc.setFontSize(7.5); doc.setTextColor(...MUTED);
    ...
    for (let i = 1; i <= pages; i++) {
      ...
      doc.text(`DentalVis  |  ${dateStr}  |  Page ${i} / ${pages}  |  For reference only - consult your dentist`,
...
function secTitle(doc, title, y, M, CW) {
  doc.setFillColor(234, 237, 227); doc.rect(M, y, CW, 7, 'F');
  doc.setFontSize(9); doc.setFont('helvetica', 'bold'); doc.setTextColor(...JADE);
