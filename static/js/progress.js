...
export function updateProgress(data, mode) {
  ...
  steps.forEach((s, i) => {
    ...
    if (s.key === step) {
      el.className = 'progress-step active'; statusEl.textContent = '進行中...';
    } else if (i < stepIdx || step === 'done') {
      el.className = 'progress-step done'; statusEl.textContent = '完成';
    } else {
      el.className = 'progress-step'; statusEl.textContent = '等待中';
    }
  });
